...
BEGIN { use_ok('Text::Sass::XS') }

my $r;
$r = Text::Sass::XS::_compile( "this is invalid sass source", {} );
is( $r->{error_status}, 1, "Failed compile sets error_status" );
like( $r->{error_message}, qr/error:/, "Failed compile sets error_message" );
is( $r->{output_string}, undef,
    "Failed compile sets output_string to undef" );

$r = Text::Sass::XS::_compile( ".valid { color: red; }", {} );
is( $r->{error_status}, 0, "Successful compile clears error_status" );
is( $r->{error_message}, undef,
    "Successful compile sets error_message to undef" );
isnt( $r->{output_string}, undef, "Successful compile sets output_string" );

# $options->{output_style}
$r = Text::Sass::XS::_compile( ".valid { color: red; }",
    { output_style => Text::Sass::XS::SASS_STYLE_NESTED } );
is( $r->{error_status}, 0,
    "output_style=>SASS_STYLE_NESTED no_error_status" );
is( $r->{error_message}, undef,
    "output_style=>SASS_STYLE_NESTED error_message is undef" );
like( $r->{output_string}, qr/\{\n/,
    "output_style=>SASS_STYLE_NESTED has returns in output" );

$r = Text::Sass::XS::_compile( "\n.valid {\n color: red; }",
    { output_style => Text::Sass::XS::SASS_STYLE_COMPRESSED } );
is( $r->{error_status}, 0,
    "output_style=>SASS_STYLE_COMPRESSED no_error_status" );
is( $r->{error_message}, undef,
    "output_style=>SASS_STYLE_COMPRESSED error_message is undef" );
...
unlike( $r->{output_string}, qr@/\*@,
    "source_comments=>0 has no added comments" );

# $options->{include_paths}
$r = Text::Sass::XS::_compile( '@import "colors"; .valid { color: $red; }',
    {} );
...
$r = Text::Sass::XS::_compile(
    '@import "colors"; .valid { color: $red; }',
    { include_paths => 't/nonexistent:t/inc' }
);
...
# $options->{image_path}
$r = Text::Sass::XS::_compile( '.valid { color: image-url("path"); }', {} );
...
is( $r->{error_message}, undef,
    "image_path w/ 2 paths error_message is undef" );
